...
is-increasing : ∀{Γ} {m n : Tm Γ ℕᶠ} → (Tm Γ (Πᶠc (Finᶠ m) (Finᶠ n))) → Ty Γ 0
is-increasing {Γ} {m} {n} f = Πᶠ (Finᶠ m)
                             (Πᶠ (Finᶠ (◁ m))
                                    (Πᶠc (gt (◁ ▼) ▼)
                                           (gt ((◁ (◁ f)) $ ◁ ▼) ((◁ (◁ f)) $ (coe (TmΓ≡ lift) ▼)))))
...
_⇒+_ {Γ} m n = Σᶠ (Πᶠc (Finᶠ m) (Finᶠ n)) (is-increasing {Γ , Πᶠc (Finᶠ m) (Finᶠ n)} {◁ m} {◁ n} ▼)


-- composition:
_∘+_ : ∀{Γ} → {l : Tm Γ ℕᶠ} → {m : Tm Γ ℕᶠ} → {n : Tm Γ ℕᶠ}
  → (Tm Γ (m ⇒+ n)) → (Tm Γ (l ⇒+ m)) → (Tm Γ (l ⇒+ n))
_∘+_ {Γ} {l} {m} {n} g f = pair (λᶠ ((◁ {!g!}) $ ((◁ (fst f)) $ ▼))) (λᶠ (◁ (snd {!!}) $ (◁ (snd f) $ ▼)))
-- pair (λᶠ ((◁ (fst {!g!})) $ ((◁ (fst f)) $ ▼))) (λᶠ ((◁ (snd {!g!})) $ (◁ (snd f) $ ▼)))
